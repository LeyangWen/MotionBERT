...
class Attention(nn.Module):
    ...
    def forward_temporal(self, q, k, v, seqlen:int=8):
        B, _, N, C = q.shape
...
class DSTformer_coreml(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
        ...
        self.att_fuse = att_fuse
        if not self.att_fuse:
            raise ValueError("att_fuse for DSTformer must be set to True for this coreml file")
    ...
    def forward(self, x, return_rep:bool=False):
        ...
        x = self.pos_drop(x)
        alphas = []
        for idx, (blk_st, blk_ts, att) in enumerate(zip(self.blocks_st, self.blocks_ts, self.ts_attn)):
            x_st = blk_st(x, F)
            x_ts = blk_ts(x, F)
